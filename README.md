...
After editing a workflow, run `npm run lint:workflows` before pushing. This uses the pinned upstream actionlint command and requires Go, which is also available on the standard CI runner. It checks Actions syntax and expression-context availability in addition to the main Node-based quality gate.
