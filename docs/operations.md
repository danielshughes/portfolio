...
In another terminal, run `npm run dev`. An unavailable local AI provider or absent Radar token must produce the documented error state, not a synthetic success. Browser-local experiments still work without live bindings.
...
CI runs every configured browser. An explicit local browser subset must be reported as such. After workflow changes run `npm run lint:workflows`; after binding changes run `npm run types:worker`. Audit dependencies, run the redacted secret scan and inspect generated assets for private content. Dry-run output and test artifacts belong outside the checkout.

## Normal deployment

1. Work on a feature branch and open a PR against `develop`.
2. The read-only required quality job always reports. Recognised documentation-only changes skip dependency/browser installation, the expensive suite and deployment; site copy and unknown files still require quality. Workflow-level PR path filters are intentionally avoided because required checks could remain pending.
3. Merge only after the actual required checks pass. Do not substitute a local result for failing CI or bypass protection.
4. A trusted `develop` push builds that exact merge commit using development settings. The deploy job uses only its branch-restricted environment secrets. PR runs cannot deploy, and an in-flight protected-branch deployment is not automatically cancelled by a newer run.
...
- Two authenticated WebSocket clients receive the same real sequence. Verify invalid messages close, sessions expire and the browser disconnects on lifecycle changes. Do not mistake preview pulses for server events.
...
| Cron and Radar      | Current five-minute development cadence is 288 ticks/day, at most five upstream Radar calls and one KV write per tick. Country rotation comes from the model list. Only development is deployed; do not count a prepared production schedule as active.                                                                                                                                                          |
| KV                  | Current maximum is 288 writes/day for development, or 576 if both approved environments later run. This is below Free's 1,000 writes/day before other account use. Reads are separate. Snapshot TTL is two hours; eligibility is one hour. [KV limits](https://developers.cloudflare.com/kv/platform/limits/)                                                                                                    |
