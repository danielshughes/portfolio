...
The mobile polish and Radar layout suites retain traces and screenshots only for failed cases. CI uploads only the dedicated test-output directory, excluding hidden files and expiring evidence after three days. These suites use authored local fixtures, never live credentials. A temporary PR-only Linux stress step repeats the two intermittent Radar interactions and stops on the first failure; remove that extra step after diagnosis, keeping failure evidence available.
