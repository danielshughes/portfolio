...
Access applies to the whole development hostname, not merely HTML. Both Workers have no public `workers.dev` or version-preview URL. Both environments use native Workers Cache for Radar only; local preview retains D1 caching. There is no origin server, Pages project, R2 asset bucket or separate API domain.
...
The response path checks a successful cache entry, then a recent validated KV snapshot, then shared error backoff, before making upstream requests. A validated snapshot also warms the existing response cache for the shorter of its remaining lifetime and `CACHE_SECONDS`, never a new freshness window. Traffic uses a time series and reported disruptions; category views use their respective summaries. Only expected fields are returned. Requests have deadlines, reject redirects and enforce response-size/event-count limits. The Radar token goes only in the server-to-Cloudflare authorisation header. Browser cookies, Access credentials and caller-provided URLs are never forwarded upstream.

The D1 fallback exists because Cloudflare states: "For Workers fronted by Cloudflare Access, the Cache API is not currently available." [Cache API documentation](https://developers.cloudflare.com/workers/runtime-apis/cache/). This limitation is about Cache API, not native Workers Cache. D1's keyspace is restricted to the configured country/view combinations and one backoff key. Request hostnames cannot create additional rows. Entries overwrite by primary key, expire logically, and never retain visitor data. Reads reduce the browser TTL to the row's remaining lifetime. Provider backoff uses D1 behind Access and location-local Cache API in production; these also serve responses if native caching is disabled. Cache failures fail closed rather than silently flooding the upstream API.

### Native Radar cache

Both deployed environments enable caching for the named `RadarData` export and select it with `RADAR_NATIVE_CACHE`. The default export explicitly disables native caching, so the public router always performs ingress admission, method/query validation, feature/credential checks and response security policy. It sends a new internal request containing only the canonical country/view URL, without caller headers, identity, host, body, range or `request.cf`. No public route exposes the data export directly. The data entrypoint repeats input validation on a miss, then reads a validated snapshot or uses the existing cold admission and provider handling.

Successful responses use native version-isolated caching instead of a second environment success cache. Provider backoff retains the environment cache described above. Snapshot TTLs count down from the original fetch time; `must-revalidate` prevents native stale-on-error fallback. Errors are `no-store`, with no uncached retry around a failing cache call. Cron/Queues keep their existing invocation-local cache and are not affected. Local preview does not simulate a native cache hit: it retains D1; runtime fixtures can test loopback dispatch, but real HIT/MISS, Access and metering need deployed evidence.

The router exposes an allowlisted `X-Radar-Cache` diagnostic when Cloudflare supplies `Cf-Cache-Status`, and records it in sampled `radar.cache.status` traces. Cache API can also supply this header when native caching is disabled; correlate it with deployed configuration and native cache metrics rather than treating any HIT as native-cache proof. The router does not manufacture a hit when the platform supplies none. The [operations guide](operations.md#native-radar-cache) owns metering and rollback checks. See [native configuration](https://developers.cloudflare.com/workers/cache/configuration/).
...
Cron retains its five-minute health cadence, but Radar collects only on alternate slots, rotating through the country list every ten minutes. The configured country list is therefore revisited every eighty minutes. This is a chosen freshness/budget tradeoff, not an assertion that Radar publishes every ten minutes. `worker/snapshots.ts` owns cadence and eligibility.

Before Radar work, an atomic D1 upsert claims the slot in the single-row `radar_collection` table. Persisted slot ordinals retain their five-minute units, so existing claims remain comparable through deployment and rollback; odd slots skip before any Radar claim, enqueue, provider call or KV write. Concurrent, repeated or older slots also skip. A claim failure stops Radar work. The claim remains after partial or failed collection: this bounded demonstration waits for the next eligible slot instead of repeating a possibly completed write. Health sampling settles independently.
...
Each admitted slot requests that country's views, validates the results, and writes at most one bundle to KV. Its invocation-local response cache coordinates backoff but cannot recycle an old edge response into a newly dated snapshot. A bundle can contain only the successful views. Snapshots remain eligible for less than two hours from each view's `fetchedAt`, covering a full rotation; KV retention is also two hours from the write. Response caching remains capped at one hour and cannot extend eligibility. Stored data still requires validation. Eventually consistent KV propagation can mean a fresh foreground request is needed. Source `updatedAt` values remain unchanged and are distinct from retrieval times.
