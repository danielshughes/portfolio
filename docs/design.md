...
The dh signature redraws on unpressed hover or keyboard-visible focus. The stroke completes after pointer/focus exit; re-entry does not restart it, and its endpoint appears only at completion. Reduced-motion changes show the complete static mark. The current touch behaviour keeps the mark static for immediate first-tap home navigation; this is a workaround, not evidence that the requested iOS tap animation is fixed. The favicon is the plain dh monogram without accent or underline.
